...
47 components across atoms, molecules and organisms, fully typed, themeable
(light/dark), with 511 tests and a 90%/85% (lines/branches) coverage gate in CI.
...
511 tests instead of being freshly invented each time.
...
| "Prototype code we'll rewrite later." | Fully typed, 511 tests, 90%/85% coverage gate, WCAG AA contrast enforced by tests. The prototype is the production code. |
